...
def get_args():
  ...
  parser.add_argument("host", type = str, help = "Give SSH server's IP address")
  group.add_argument("-u", "--user", type = str, help = "Give a single user name")
  group.add_argument("-U", "--userlist", type = str, help = "Give a file containing a list of usernames")
  parser.add_argument("-e", "--enumerated", action = "store_true", help = "Display enumerated users")
  parser.add_argument("-s", "--silent", action = "store_true", help = "Like -e, but just the user names will be written to stdout (no banner, or anything)")
